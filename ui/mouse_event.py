...
class GraphicsScene(QGraphicsScene):
    def __init__(self, mode_list, parent=None):
        ...
        self.sketch_points = []
        self.stroke_points = []
        self.sketch_more_points = []
        self.stroke_more_points = []
        ...
        if self.mouse_clicked:
            if self.modes[0] == 1:
                ...
                if self.prev_pt:
                    ...
                    pts['prev'] = (int(self.prev_pt.x()),int(self.prev_pt.y()))
                    pts['curr'] = (int(event.scenePos().x()),int(event.scenePos().y()))
                    self.sketch_more_points.append(pts)
                ...
                if self.prev_pt:
                    ...
                    pts['curr'] = (int(event.scenePos().x()),int(event.scenePos().y()))
                    pts['color'] = self.stk_color
                    self.stroke_more_points.append(pts)
    ...
    def drawMask(self, prev_pt, curr_pt):
        ...
        lineItem.setPen(QPen(Qt.white, 6, Qt.SolidLine)) # rect
            ...
            if len(self.items())>=9:
                for i in range(8):
                    ...
                    if self.history[-1] == 0:
                        ...
                        self.sketch_more_points.pop()
                        self.history.pop()
                    elif self.history[-1] == 4:
                        self.stroke_more_points.pop()
                        self.history.pop()
                    elif self.history[-1] == 5:
                        self.history.pop()
