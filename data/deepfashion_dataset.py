...
class DeepFashionDataset(Pix2pixDataset):
    ...
    def get_paths(self, opt):
        ...
        lines = open('./datasets/deepfashion_train_test.txt').readlines()
        for l in lines:
            sub_path, tag = l.split()[0], l.split()[-1]
            if tag == opt.phase or (tag in ['train', 'test'] and opt.phase == 'train_test'):
                image_path = os.path.join(opt.dataroot, 'img_320_512_image', sub_path)
                if opt.phase == 'test' and opt.stage == 25:
                    label_path = os.path.join(opt.dataroot, 'img_320_512_parsing_synthesized', sub_path).replace('.jpg', '.png')
                    # web_dir = os.path.join(opt.results_dir, opt.name, '%s_%s' % (opt.phase, opt.which_epoch),
                    #                        'images', 'synthesized_parsing_onechannel',
                    #                        'img_320_512_parsing_synthesized')
                    # label_path = os.path.join(web_dir, sub_path).replace('.jpg', '.png')
                else:
                    label_path = os.path.join(opt.dataroot, 'img_320_512_parsing', sub_path).replace('.jpg', '_gray.png')
                edge_path = os.path.join(opt.dataroot, 'img_320_512_edge', sub_path)
                color_path = os.path.join(opt.dataroot, 'img_320_512_color', sub_path)
